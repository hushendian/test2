...
import com.domuiteam.domui.widget.Toast.style.ToastBlackStyle;
...
public final class ToastUtils {
    ...
    public static void show(Object object) {
        show(object != null ? object.toString() : "");
    }
...
//        if (text == null || "".equals(text.toString())) return;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            gravity =
                    Gravity.getAbsoluteGravity(
                            gravity, sToast.getView().getResources().getConfiguration().getLayoutDirection());
        }
     ...
     *
     * @param style 样式实现类，框架已经实现三种不同的样式 黑色样式：{@link ToastBlackStyle}
     */
    ...
    private static TextView createTextView(Context context) {
        ...
        textView.setPadding(
                (int)
                        TypedValue.applyDimension(
                                TypedValue.COMPLEX_UNIT_DIP,
                                sDefaultStyle.getPaddingLeft(),
                                context.getResources().getDisplayMetrics()),
                (int)
                        TypedValue.applyDimension(
                                TypedValue.COMPLEX_UNIT_DIP,
                                sDefaultStyle.getPaddingTop(),
                                context.getResources().getDisplayMetrics()),
                (int)
                        TypedValue.applyDimension(
                                TypedValue.COMPLEX_UNIT_DIP,
                                sDefaultStyle.getPaddingRight(),
                                context.getResources().getDisplayMetrics()),
                (int)
                        TypedValue.applyDimension(
                                TypedValue.COMPLEX_UNIT_DIP,
                                sDefaultStyle.getPaddingBottom(),
                                context.getResources().getDisplayMetrics()));
    ...
    /**
     * 检查通知栏权限有没有开启 参考SupportCompat包中的方法：
